...
#if LV_FS_USE_SDCARD
static FATFS *fat[2];
#else /* LV_FS_USE_SDCARD */
static FATFS *fat[1];
#endif /* LV_FS_USE_SDCARD */
...
static void fs_init(void) {
    ...
    if (res == FR_OK) {
        LV_FAT_DBG("NOR flash mount success. ");
    } else if (res == FR_NO_FILESYSTEM) {
        LV_FAT_DBG("No file system in NOR flash. Formatting... ");
        res = f_mkfs("0:", NULL, NULL, FF_MAX_SS);

        if (res == FR_OK) {
            f_setlabel((const TCHAR *)"0:STM32 NOR");
            LV_FAT_DBG("NOR flash format finish. ");
        } else {
            LV_FAT_DBG("NOR flash format failed. ");
        }
    } else {
        LV_FAT_DBG("NOR flash mount failed. ");
        lv_mem_free(fat[0]);
    }
