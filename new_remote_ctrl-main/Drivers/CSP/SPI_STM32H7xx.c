...
static DMA_HandleTypeDef spi5_dmarx_handle = {
    ...
    .Init = {.Request = DMA_REQUEST_SPI5_RX,     
             .FIFOMode = DMA_FIFOMODE_DISABLE,
             .Direction = DMA_PERIPH_TO_MEMORY,
             .PeriphInc = DMA_PINC_DISABLE,
             .MemInc = DMA_MINC_ENABLE,
...
static DMA_HandleTypeDef spi6_dmarx_handle = {
    .Instance = CSP_BDMA_CHANNEL(SPI6_RX_BDMA_CHANNEL),
    .Init = {.Request = BDMA_REQUEST_SPI6_RX,     
             .FIFOMode = DMA_FIFOMODE_DISABLE,
             .Direction = DMA_PERIPH_TO_MEMORY,
             .PeriphInc = DMA_PINC_DISABLE,
             .MemInc = DMA_MINC_ENABLE,
...
static DMA_HandleTypeDef spi6_dmatx_handle = {
    .Instance = CSP_BDMA_CHANNEL(SPI6_TX_BDMA_CHANNEL),
    .Init = {.Request = BDMA_REQUEST_SPI6_TX,
             .FIFOMode = DMA_FIFOMODE_DISABLE,
             .Direction = DMA_MEMORY_TO_PERIPH,
             .PeriphInc = DMA_PINC_DISABLE,
             .MemInc = DMA_MINC_ENABLE,
...
#if QUADSPI_MDMA
MDMA_HandleTypeDef qspi_mdma_handle = {
 ...
 *  @retval - 2: `QSPI_INIT_MDMA_FAIL`: QSPI MDMA init failed.
...
uint8_t qspi_init(uint32_t clock_mode, uint32_t flash_size) {
    ...
    HAL_NVIC_SetPriority(MDMA_IRQn, QUADSPI_MDMA_IT_PRIORITY,
                         QUADSPI_MDMA_IT_SUB);
...
void MDMA_IRQHandler(void) {
    HAL_MDMA_IRQHandler(&qspi_mdma_handle);
}
#endif /* QUADSPI_MDMA */
...
#if QUADSPI_MDMA
    if (HAL_MDMA_DeInit(&qspi_mdma_handle) != HAL_OK) {
        return QSPI_DEINIT_MDMA_FAIL;
    }
    HAL_NVIC_DisableIRQ(MDMA_IRQn);
    qspi_handle.hmdma = NULL;
#endif /* QUADSPI_MDMA */
