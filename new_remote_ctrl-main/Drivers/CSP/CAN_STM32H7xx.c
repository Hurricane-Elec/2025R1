...
uint8_t fdcan1_init(uint32_t baud_rate, uint32_t fd_mode, uint32_t prop_delay) {
    ...
    uint32_t fdcan_clock_freq;
    switch (__HAL_RCC_GET_FDCAN_SOURCE()) {
        case RCC_FDCANCLKSOURCE_HSE: {
            fdcan_clock_freq = HSE_VALUE;
        } break;

        case RCC_FDCANCLKSOURCE_PLL: {
            PLL1_ClocksTypeDef pll1_clock;
            HAL_RCCEx_GetPLL1ClockFreq(&pll1_clock);
            fdcan_clock_freq = pll1_clock.PLL1_Q_Frequency;
        } break;

        case RCC_FDCANCLKSOURCE_PLL2: {
            PLL2_ClocksTypeDef pll2_clock;
            HAL_RCCEx_GetPLL2ClockFreq(&pll2_clock);
            fdcan_clock_freq = pll2_clock.PLL2_Q_Frequency;
        } break;
    ...
    if (can_rate_calc(baud_rate * 1000, prop_delay, fdcan_clock_freq, &prescale,
                      &tsjw, &tbs1, &tbs2) == 1) {
        return CAN_INIT_RATE_ERR;
    }
...
uint8_t can_rate_calc(uint32_t baud_rate, uint32_t prop_delay,
    ...
    *tseg1 += prop_seg;
    ...
    if (*tseg2 > 128) {
        /* STM32H7 CAN BS2 range: 2-128 */
        *tseg2 = 128;
        *tseg1 = tq_remain - *tseg2;
    }
    if (*tseg1 > 256) {
        /* STM32H7 CAN BS1 range: 2-256 */
        return 1;
    }
