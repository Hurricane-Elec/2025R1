 ...
 * @version     V1.1
 * @date        2022-04-20
 * @brief       lcd_ex.h 存放各个LCD驱动IC的寄存器初始化部分代码, 以简化 lcd.c
 * @warning     私有头文件, 其他地方不要包括含文件!
 ...
 *
 */
static void lcd_ex_nt35510_reginit(void) {
...
static void lcd_ex_ili9806_reginit(void) {
    ...
    lcd_wr_regno(
        0xED); /* EN_volt_reg VGMP / VGMN /VGSP / VGSN voltage to output */
    lcd_wr_data(0x7F);
    lcd_wr_data(0x0F);
...
static void lcd_ex_ssd1963_reginit(void) {
    lcd_wr_regno(
        0xE2); /* Set PLL with OSC = 10MHz (hardware),	Multiplier N = 35,
