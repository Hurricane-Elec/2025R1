...
void step_motor_init(step_motor_handle_t *handle) {
    ...
    step_motor_timer_init(handle);

    step_motor_disable(handle);
}

/**
 * @brief 步进电机脉冲定时器pwm初始化
 * 
 * @param handle 句柄
 */
void step_motor_timer_init(step_motor_handle_t *handle) {
    ...
    STEP_MOTOR_TIMER_CLK_ENABLE();
    TIM_OC_InitTypeDef tim_pwm_config = {0};

    handle->htim->Instance = STEP_MOTOR_TIMER;
...
/**
 * @brief 步进电机运动一段距离（使用中断）
 *
 * @param handle 句柄
 * @param pulse_num 脉冲个数
 */
void step_motor_run_IT(step_motor_handle_t *handle, int32_t pulse_num) {
    if (handle == NULL) {
        return;
    }
